...
void tool_fxc(const vdfastvector<const wchar_t*>& args, const vdfastvector<const wchar_t*>& switches, bool amd64) {
	...
	const wchar_t* filename = args[0];

	printf("Asuka: Compiling effect file (Direct3D): %ls -> %ls.\n", filename, args[1]);
	...
	HRESULT hr = D3DXCreateEffectFromFileW(pDevice, filename, NULL, NULL, flags, NULL, ~pEffect, ~pErrors);

	if (FAILED(hr)) {
		printf("Effect compilation failed for \"%ls\"\n", filename);
	...
	FILE *f = nullptr;
	errno_t err = _wfopen_s(&f, args[1], L"w");
	if (err) {
		printf("Couldn't open %ls for write\n", args[1]);
		exit(10);
	}

	fprintf(f, "// Effect data auto-generated by Asuka from %ls. DO NOT EDIT!\n", VDFileSplitPath(filename));
