...
void tool_fxc10(const vdfastvector<const wchar_t*>& args, const vdfastvector<const wchar_t*>& switches) {
	...
	printf("Asuka: Compiling effect file (D3D10): %ls -> %ls.\n", filename, args[1]);

	VDFile f(filename);
	FILE *fo = nullptr;
	errno_t err = _wfopen_s(&fo, args[1], L"w");
	if (err) {
		printf("Asuka: Unable to open for write: %ls\n", args[1]);
		exit(10);
	}

	try {
		fprintf(fo, "// Generated by Asuka from %ls. DO NOT EDIT.", filename);
		...
		while(const char *line = ts.GetNextLine()) {
			...
			while(!targets.empty()) {
				...
				FXC10IncludeHandler includeHandler(VDFileSplitPathLeft(filenameA).c_str());

				vdrefptr<ID3D10Blob> shader;
				vdrefptr<ID3D10Blob> errors;
				HRESULT hr = D3DCompile(buf.data(), len, filenameA.c_str(), macros, &includeHandler, function_name.c_str(), compile_target.c_str(), 0, 0, ~shader, ~errors);

				if (FAILED(hr)) {
					printf("Effect compilation failed for \"%ls\" with target %s (hr=%08x)\n", filename, compile_target.c_str(), hr);
